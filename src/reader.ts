...
//
// A few runtimes ship without the euc-kr table (Node built with small-icu, some
// edge runtimes), which is why the constructor is wrapped: there the decoder is
// simply absent and every name goes through the Windows-1252 path.
const cp949Strict = (() => {
  try {
...
export function readEntityName(buf: Uint8Array): string {
  let start = 0;
  while (start < buf.length && (buf[start] === 0x00 || buf[start] === 0xff)) {
    start++;
  }
  let end = buf.length;
